    ...
    try:
        ...
        if main_function == "1. 投手的數據分析":
            ...
            if len(all_pitchers) == 0:
                ...
                # 左右並排：左邊畫圓餅圖，右邊放詳細出賽狀況
                graph_col, table_col = st.columns([2, 3]) # 控制比例：左邊寬度 2，右邊寬度 3
                ...
                with table_col:
                    ...
                    # 💡 自動尋找並動態挑選適合顯示的明細欄位
                    # 如果你的 Excel 有『對手』、『比賽日期』等欄位，程式會自動抓，沒有也不會當機
                    available_cols = p_df.columns.tolist()
                    
                    # 定義我們想呈現的理想欄位順序（如果存在的話）
                    ...
                    # 如果連對手都沒定義，就顯示除了『真實局數』以外的所有既有欄位
