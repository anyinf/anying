        ...
        if main_function == "1. 投手的數據分析":
            ...
            if len(all_pitchers) == 0:
                ...
                with table_col:
                    ...
                    target_show_cols = ['比賽日期', '對手', '局數', '總球數', '好球數', '壞球數', '奪三振', '四死球', '被安打', '自責分', '最速']
                    actual_show_cols = [c for c in target_show_cols if c in available_cols]
                    
                    if len(actual_show_cols) <= 2: 
                        actual_show_cols = [c for c in available_cols if c != '真實局數']
        ...
        # ==================== 功能二：投手春聯表現 ====================
            ...
            if len(all_pitchers) == 0:
                ...
                if not summary_df.empty:
                    ...
                    # 🌟 2. 球速雙維度深度分析區塊 (已修正潛在崩潰語法)
                    st.markdown(f"## 📊 投手球速多維度追蹤：歷場大趨勢 vs 單場續航力")
                    
                    p_trend_df = df[df['出賽球員'] == selected_pitcher_2].copy()
                    p_trend_df = p_trend_df[p_trend_df['最速'] > 0] 
                    
                    if not p_trend_df.empty:
                        col_long, col_single = st.columns(2)
                        
                        # ---------------- 左區塊：歷場比賽大趨勢 ----------------
                        with col_long:
                            st.markdown("#### 📅 【維度一】歷場出賽最高球速變化 (宏觀趨勢)")
                            if '比賽日期' in p_trend_df.columns:
                                x_col = '比賽日期'
                                p_trend_df = p_trend_df.sort_values(by='比賽日期')
                            elif '對手' in p_trend_df.columns:
                                x_col = '對手'
                            else:
                                # 安全寫法：修正潛在崩潰點
                                p_trend_df['出賽場次'] = [f"第 {i+1} 場" for i in range(len(p_trend_df))]
                                x_col = '出賽場次'
                                
                            fig_long = px.line(
                                p_trend_df, x=x_col, y='最速', markers=True,
                                title=f"{selected_pitcher_2} 春聯各場比賽最速走勢",
                                labels={x_col: '出賽識別 (時間軸)', '最速': '最快球速 (km/h)'}
                            )
                            fig_long.update_traces(line=dict(color='#FF4B4B', width=3))
                            fig_long.update_yaxes(range=[p_trend_df['最速'].min() - 5, p_trend_df['最速'].max() + 5])
                            st.plotly_chart(fig_long, use_container_width=True)
                            st.caption("🔍 解讀：觀測球員隨著賽程推進，長期的球速體能有無下滑。")
                        
                        # ---------------- 右區塊：單場內隨著局數變化 ----------------
                        with col_single:
                            st.markdown("#### ⚾ 【維度二】單場比賽內球速續航力 (微觀分析)")
                            
                            # 建立穩固的單場選擇標籤
                            if '比賽日期' in p_trend_df.columns and '對手' in p_trend_df.columns:
                                p_trend_df['場次標籤'] = p_trend_df['比賽日期'].astype(str) + " vs " + p_trend_df['對手'].astype(str)
                            elif '對手' in p_trend_df.columns:
                                p_trend_df['場次標籤'] = p_trend_df['對手'].astype(str)
                            else:
                                p_trend_df['場次標籤'] = [f"出賽次序 {i+1}" for i in range(len(p_trend_df))]
                                
                            unique_games = p_trend_df['場次標籤'].unique()
                            selected_game = st.selectbox("請選擇想深入觀測的單場比賽：", unique_games)
                            
                            single_game_df = p_trend_df[p_trend_df['場次標籤'] == selected_game]
                            
                            # 判斷是單場多局明細還是一場一筆數據
                            if len(single_game_df) > 1 and '局數' in single_game_df.columns:
                                single_game_df = single_game_df.sort_values(by='局數')
                                fig_single = px.line(
                                    single_game_df, x='局數', y='最速', markers=True,
                                    title=f"{selected_game} - 各局球速續航力折線",
                                    labels={'局數': '局數 (進程)', '最速': '該局最速 (km/h)'}
                                )
                                fig_single.update_traces(line=dict(color='#1C83E1', width=3))
                            else:
                                y_bar_val = '局數' if '局數' in single_game_df.columns else '真實局數'
                                fig_single = px.bar(
                                    single_game_df, 
                                    x=single_game_df.index.astype(str), # 避免空軸故障
                                    y='最速',
                                    title=f"{selected_game} - 當場最高球速",
                                    labels={'x': '場次定位', '最速': '最高球速 (km/h)'},
                                    text_auto=True
                                )
                                fig_single.update_traces(marker_color='#1C83E1')
                                if y_bar_val in single_game_df.columns:
                                    fig_single.update_layout(hovermode="x", title=f"{selected_game} - 當場最高球速 (投球局數: {single_game_df[y_bar_val].values[0]})")
                                
                            st.plotly_chart(fig_single, use_container_width=True)
                            st.caption("🔍 解讀：觀測投手在「單一場比賽」中，體能續航力是否能撐到比賽後半段。")
                    else:
                        st.info(f"💡 目前找不到選手【{selected_pitcher_2}】的球速明細資料。")
