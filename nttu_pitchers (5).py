    ...
    try:
        ...
        if main_function == "1. 投手的數據分析":
            ...
            if len(all_pitchers) == 0:
                ...
                # 連動優化：將 Selectbox 放回主畫面，操作更直覺
                selected_pitcher_1 = st.selectbox("請選擇要分析的東大投手：", all_pitchers, key="func_1")
            ...
            if len(all_pitchers) == 0:
                ...
                if not summary_df.empty:
                    ...
                    summary_df['焦點標記'] = summary_df['投手'].apply(
                        lambda x: target_label if str(x) == str(selected_pitcher_2) else '東大其他球員'
                    )
                    ...
                    fig_scatter = px.scatter(
                        ...
                        color_discrete_map={target_label: '#FF4B4B', '東大其他球員': '#1C83E1'},
                        hover_data=['累積投球局數']
                    )
                    
                    fig_scatter.update_traces(textposition='top center')
                    fig_scatter.update_yaxes(autorange="reversed") # 防禦率倒轉，越低（強）越在上方
                    st.plotly_chart(fig_scatter, use_container_width=True)
                    
                    # 呈現對照數據表，方便點閱與排序
                    st.markdown("### 📊 全隊投手春季聯賽數據總覽")
                    st.dataframe(summary_df.drop(columns=['焦點標記']), use_container_width=True)
                else:
