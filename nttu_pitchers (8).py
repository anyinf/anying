            ...
            if len(all_pitchers) == 0:
                ...
                if not summary_df.empty:
                    ...
                    fig_scatter = px.scatter(
                        ...
                        color_discrete_map={target_label: '#FF4B4B', 'Dark2': '#1C83E1'},
                        hover_data=['累積投球局數']
                    )
                    ...
                    # 🌟 2. 歷場最高球速變化趨勢（已合併比賽日期與安全防護機制）
                    st.markdown(f"### 📅 歷場出賽最高球速變化走勢 (觀測對象：{selected_pitcher_2})")
                    
                    p_trend_df = df[df['出賽球員'] == selected_pitcher_2].copy()
                    p_trend_df = p_trend_df[p_trend_df['最速'] > 0] # 濾除無球速髒資料
                    
                    if not p_trend_df.empty:
                        # 檢查並確保比賽日期是以字串型態排序與顯示，避免 Plotly 時間軸軸線錯亂
                        if '比賽日期' in p_trend_df.columns:
                            p_trend_df['比賽日期'] = p_trend_df['比賽日期'].astype(str)
                        ...
                        fig_long = px.line(
                            p_trend_df, 
                            x=x_col, 
                            y='max_speed' if 'max_speed' in p_trend_df.columns else '最速', 
                            markers=True,
                            title=f"【{selected_pitcher_2}】歷場春季聯賽最高球速變化走勢",
                            labels={x_col: '比賽日期 (依賽程排序)', '最速': '最快球速 (km/h)', 'max_speed': '最快球速 (km/h)'},
                        ...
                        # 動態優化 Y 軸範圍，使波動比例視覺上更合理
                        current_speed_col = '最速' if '最速' in p_trend_df.columns else 'max_speed'
                        min_spd = p_trend_df[current_speed_col].min()
                        max_spd = p_trend_df[current_speed_col].max()
                        fig_long.update_yaxes(range=[max(0, min_spd - 5), max_spd + 5])
                        
                        st.plotly_chart(fig_long, use_container_width=True)
                        st.caption("🔍 視覺化解讀：此圖表追蹤投手在整個春季聯賽期間的體力狀態。若折線隨著時間軸推進（向右）呈現下滑，表示可能存在長期累積疲勞。")
                    else:
